...
# FAST ROM: no boot menu, limine timeout 0 → Hostess7 default entry
setsid /usr/bin/qemu-system-x86_64 \
  "${KVM[@]}" -m "${KILROY_MEM:-4096}" -smp "${KILROY_SMP:-4}" \
  -name Spear-Hostess7-HARD \
  -machine q35 \
  -boot order=c,menu=off,strict=on,reboot-timeout=0 \
  "${DISPLAY_ARGS[@]}" \
...
echo "pid=$!  Spear-Hostess7-HARD (ROM→Hostess7 FAST)"
echo "serial=$SERIAL"
echo "disk=$IMG"
