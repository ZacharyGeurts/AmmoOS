...
# hostess7 (default FAST·HARD) | field | normal | fielddrive | war | debug
SPEAR_BOOT_MODE="${SPEAR_BOOT_MODE:-hostess7}"
...
COMMON=(
  ...
  # FAST ROM: no boot menu delay, no reboot hang, direct when -kernel used
  -boot menu=off,strict=on,reboot-timeout=0
...
if [[ "$DIRECT" == "1" ]]; then
  ...
  # Hostess7 = FAST·HARD default (ROM → kernel → H7). Direct -kernel skips ISO menu.
  MASK_LEAN="lightdm.service,accounts-daemon.service,ModemManager.service,cups.service,cups-browsed.service,avahi-daemon.service,blueman-mechanism.service,openvpn.service,touchegg.service,kerneloops.service,casper-md5check.service,ubiquity.service,mintsystem.service,ssl-cert.service,switcheroo-control.service,apport.service,power-profiles-daemon.service,thermald.service,bluetooth.service,rsyslog.service,cron.service,anacron.service,whoopsie.service"
  case "$SPEAR_BOOT_MODE" in
    hostess7|h7|war-fast)
      APPEND="boot=casper uuid=${UUID} username=spear hostname=spear spear_mode=hostess7 spear_harden=1 spear_fast=1 spear_war=1 field=1 i2=1 always_war=1 hostess7=1 systemd.unit=multi-user.target systemd.wants=spear-hostess7-boot.service,spear-i2-war.service,spear-wartime.service quiet loglevel=2 systemd.show_status=false fsck.mode=skip raid=noautodetect noresume tsc=reliable apparmor=1 security=apparmor pti=on systemd.mask=${MASK_LEAN} console=tty0 console=ttyS0,115200n8 vga=normal ---"
      ;;
    field|fast)
      APPEND="boot=casper uuid=${UUID} username=spear hostname=spear spear_mode=field spear_harden=1 spear_fast=1 hostess7=1 i2=1 always_war=1 systemd.unit=multi-user.target systemd.wants=spear-hostess7-boot.service quiet loglevel=3 systemd.show_status=false fsck.mode=skip raid=noautodetect noresume tsc=reliable apparmor=1 security=apparmor pti=on systemd.mask=${MASK_LEAN} console=tty0 console=ttyS0,115200n8 vga=normal ---"
      ;;
    normal|desktop)
      ...
      ;;
    *)
      APPEND="boot=casper uuid=${UUID} username=spear hostname=spear spear_mode=${SPEAR_BOOT_MODE} spear_harden=1 hostess7=1 i2=1 always_war=1 quiet loglevel=3 fsck.mode=skip raid=noautodetect noresume apparmor=1 security=apparmor pti=on console=tty0 console=ttyS0,115200n8 vga=normal ---"
      ;;
  esac
  echo "SPEAR_BOOT_MODE=$SPEAR_BOOT_MODE  (direct kernel — skip ISO ROM menu)"
  # -kernel path: SeaBIOS hands off immediately (no isolinux wait)
  ...
  # ISO boot: still no menu delay (isolinux timeout 0)
  exec qemu-system-x86_64 \
    "${COMMON[@]}" \
