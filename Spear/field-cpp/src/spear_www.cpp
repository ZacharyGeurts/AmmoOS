...
// Default: field bind (0.0.0.0) · AmmoOS / Internet 2.0 — not 127-only
...
static bool blocked_script_ext(const std::string& path) {
  auto ends = [&](const char* ext) -> bool {
  ...
  // Interpreters / scripts never served — Python, QBasic, shells, etc. DISARMED
  return ends(".py") || ends(".pyw") || ends(".pyc") || ends(".pyo") || ends(".pyx") ||
         ends(".bas") || ends(".qb") || ends(".qbas") || ends(".bi") ||
         ends(".sh") || ends(".bash") || ends(".zsh") || ends(".csh") ||
         ends(".pl") || ends(".pm") || ends(".rb") || ends(".php") || ends(".cgi") ||
         ends(".ps1") || ends(".cmd") || ends(".bat") || ends(".vbs") || ends(".lua") ||
         ends(".aml") || ends(".ts") || ends(".tsx");
}
...
static bool safe_path(const std::string& rel) {
  ...
  if (blocked_script_ext(rel)) return false;
  // engine trees never exposed
  std::string low = rel;
  for (char& c : low)
    if (c >= 'A' && c <= 'Z') c = static_cast<char>(c - 'A' + 'a');
  if (low.find("lib/") != std::string::npos || low.find("scripts/") != std::string::npos ||
      low.find("__pycache__") != std::string::npos)
    return false;
  return true;
}
...
static std::string api_status(const std::string& root) {
  ...
  body += std::string("  \"bind\": \"") + spear::field_bind_ip() + ":9490\",\n";
  ...
  body += "  \"shard_cache\": 0,\n";
  body += "  \"planet\": \"http://" + spear::field_advertise_host() + ":9600/planet/status\",\n";
...
static void handle_client(int cfd, const std::string& root) {
  ...
  // ── Static (no scripts ever) ────────────────────────────────────────────
  if (!path.empty() && path[0] == '/') path = path.substr(1);
  if (!safe_path(path)) {
    send_raw(cfd, 403, "application/json",
             "{\"error\":\"forbidden\",\"scripts\":\"FORBIDDEN\",\"interpreters\":\"DISARMED\"}\n");
    return;
  }
  ...
  if (blocked_script_ext(fpath)) {
    send_raw(cfd, 403, "application/json",
             "{\"error\":\"forbidden\",\"reason\":\"script_extension\"}\n");
    return;
  }
