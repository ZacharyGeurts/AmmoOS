...
bool needs_priv(const std::string& cmd) {
  ...
  return cmd == "ffat-ensure" || cmd == "ffat-force" || cmd == "ffat-put" || cmd == "ffat-get" ||
         cmd == "field1-backup" || cmd == "field1-claim" || cmd == "purge-fox";
}
...
bool autoelevate_begin(const std::string& cmd, std::string& err) {
  ...
  // NEVER sudo / pkexec / pkill theater — setuid spear or file caps only.
  err =
      "autoelevate unavailable: setuid spear or file caps only "
      "(install -o root -m 4755 spear) — NEVER sudo, NEVER pkexec, NEVER polkit prompt";
  return false;
}
...
std::string elevate_status_line() {
  ...
  std::snprintf(buf, sizeof buf,
                "uid=%d euid=%d elevated=%s model=autoelevate(setuid-or-caps) "
                "never=sudo,pkexec,polkit,pkill allowlist=field-disk-ops",
