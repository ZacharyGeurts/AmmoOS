...
int main(int argc, char** argv) {
  ...
  // purge-fox — delete Firefox/Mozilla icon files system-wide (autoelev setuid)
  if (cmd == "purge-fox") {
    if (need_elev(cmd)) return 77;
    int removed = 0, failed = 0;
    // iterative stack walk — C++ only, no scripts
    std::vector<std::string> stack;
    stack.emplace_back("/usr/share/icons");
    stack.emplace_back("/usr/share/pixmaps");
    stack.emplace_back("/usr/lib/firefox/browser/chrome/icons");
    stack.emplace_back("/usr/lib/firefox/icons");
    if (const char* home = std::getenv("HOME")) {
      stack.emplace_back(std::string(home) + "/.local/share/icons");
      stack.emplace_back(std::string(home) + "/.local/share/pixmaps");
      stack.emplace_back(std::string(home) + "/.icons");
    }
    while (!stack.empty()) {
      std::string root = stack.back();
      stack.pop_back();
      DIR* d = ::opendir(root.c_str());
      if (!d) continue;
      while (dirent* ent = ::readdir(d)) {
        if (std::strcmp(ent->d_name, ".") == 0 || std::strcmp(ent->d_name, "..") == 0) continue;
        std::string path = root + "/" + ent->d_name;
        struct stat st {};
        ...
        // basename fox check
        char low[512];
        std::snprintf(low, sizeof low, "%s", ent->d_name);
        for (char* p = low; *p; ++p)
          *p = static_cast<char>(std::tolower(static_cast<unsigned char>(*p)));
        if (std::strstr(low, "firefox") == nullptr && std::strstr(low, "mozilla-firefox") == nullptr)
          continue;
        const char* dot = std::strrchr(low, '.');
        if (!dot) continue;
        if (std::strcmp(dot, ".png") && std::strcmp(dot, ".svg") && std::strcmp(dot, ".xpm") &&
            std::strcmp(dot, ".ico") && std::strcmp(dot, ".svgz"))
          continue;
    ...
    std::printf("purge-fox DONE removed=%d failed=%d euid=%d\n", removed, failed,
                static_cast<int>(::geteuid()));
    // cache refresh without shell scripts — invoke gtk helper directly
    if (fork() == 0) {
      execlp("gtk-update-icon-cache", "gtk-update-icon-cache", "-f", "-t",
             "/usr/share/icons/hicolor", static_cast<char*>(nullptr));
      _exit(127);
    }
    return failed ? 1 : 0;
  }
