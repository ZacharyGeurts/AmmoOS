...
static void write_global_rekill(int lifetime, int cycle_kills, int seen, int killed) {
  ...
  body += "  \"policy\": \"COOKED · Field UDP fries global servers to WALL OUTLET · nothing left · no storage · no room · every_kill_gets_rekill · God Bless\",\n";
...
static void write_angel() {
  // Prefer full Angel Meld (CHIPs · Plate · Meld) from spear-angel-seal if present.
  // That plate seals STACK + ISO and marks interpreters DISARMED.
  const std::string meld_candidates[] = {
      spear::www_dir() + "/angel-stack-seal.json",
      spear::home_dir() + "/.local/share/spear/angel-stack-seal.json",
      spear::home_dir() + "/Desktop/SG/Spear/data/shot-certainty/angel-stack-seal.json",
  };
  std::string stack_seal_body;
  for (const auto& c : meld_candidates) {
    stack_seal_body = spear::read_file(c.c_str(), 1 << 18);
    if (!stack_seal_body.empty() && spear::contains(stack_seal_body, "angel-stack-seal")) break;
  }
  std::string iso_seal_body = spear::read_file((spear::www_dir() + "/angel-iso-seal.json").c_str(), 1 << 18);
  if (iso_seal_body.empty())
    iso_seal_body =
        spear::read_file((spear::home_dir() + "/.local/share/spear/angel-iso-seal.json").c_str(), 1 << 18);
  ...
  bool component_sealed =
      spear::contains(raw, "\"sealed\": true") || spear::contains(raw, "\"sealed\":true");
  bool eye_sealed =
      spear::contains(eye, "\"sealed\": true") || spear::contains(eye, "\"sealed\":true");
  bool stack_sealed = spear::contains(stack_seal_body, "\"sealed\": true") ||
                      spear::contains(stack_seal_body, "\"status\": \"SEALED_UP\"");
  bool iso_sealed = spear::contains(iso_seal_body, "\"sealed\": true") ||
                    spear::contains(iso_seal_body, "\"status\": \"SEALED_UP\"");

  auto pull_hex = [](const std::string& j, const char* key) -> std::string {
  ...
  std::string root_seal = pull_hex(stack_seal_body, "root_seal");
  if (root_seal.empty()) root_seal = pull_hex(raw, "root_seal");
  std::string iso_root = pull_hex(iso_seal_body, "root_seal");
  ...
  const bool sealed = (stack_sealed || component_sealed) && eye_sealed;
  const bool full = stack_sealed && iso_sealed && eye_sealed;
  const std::string ts = spear::now_z();
  std::string body;
  body += "{\n";
  body += "  \"schema\": \"angel-seal-status/v3\",\n";
  body += "  \"ts\": \"" + ts + "\",\n";
  body += "  \"sealed\": " + std::string(full || sealed ? "true" : "false") + ",\n";
  body += "  \"stack_sealed\": " + std::string(stack_sealed || component_sealed ? "true" : "false") + ",\n";
  body += "  \"iso_sealed\": " + std::string(iso_sealed ? "true" : "false") + ",\n";
  ...
  body += "  \"rank\": \"ANGEL\",\n";
  body += "  \"status\": \"" +
          std::string(full ? "SEALED_UP" : (sealed ? "PARTIAL" : "UNSEALED")) + "\",\n";
  body += "  \"stack\": \"C++\",\n";
  body += "  \"planes\": [\"CHIPs\", \"Plate\", \"Meld\"],\n";
  ...
  body += "  \"iso_root_seal\": \"" + iso_root + "\",\n";
  body += "  \"interpreters\": \"DISARMED\",\n";
  body += "  \"dangerous_languages\": false,\n";
  body += "  \"rule\": \"Python, QBasic, and other high-level runtimes never dangerous to us; engine is "
          "C++ or lower.\",\n";
  body += "  \"dns_dhcp_related\": [\"znetwork\", \"field_dns\"],\n";
  body += "  \"seal_path\": \"" + seal_path + "\",\n";
  body += "  \"final_eye_path\": \"" + eye_path + "\",\n";
  body += "  \"stack_plate\": \"angel-stack-seal.json\",\n";
  body += "  \"iso_plate\": \"angel-iso-seal.json\",\n";
  body += "  \"note\": \"Angel Meld CHIPs+Plate+Meld · component seal multi-doc ignored for trailing "
          "control-panel\"\n";
  body += "}\n";
