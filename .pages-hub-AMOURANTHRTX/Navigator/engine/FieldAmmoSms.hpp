...
#include "AmmoSms/FieldSmsCore.hpp"
...
namespace FieldAmmoSmsConfig {

inline const char* kCfgPath = "C:\\SMS\\AMMOSMS.CFG";
inline const char* kRomDir = "C:\\SMS\\";
...
inline void load(Options& o = g) noexcept {
    FieldAmmoEmu::loadConfig(kCfgPath, o, [](FieldAmmoEmu::BaseOptions& base, const char* key, const char* val) {
        ...
        if (std::strcmp(key, "ntsc") == 0) so.ntsc = std::atoi(val) != 0;
    });
}
...
inline void paintPage(FieldRtxWidgets::Ui& ui, int pg, int s, FieldAmmoEmu::BaseOptions& o, int& y) noexcept {
    if (pg == 0) FieldAmmoEmu::paintAudioRows(ui, s, o, y);
        ...
        const char* lines[] = {
            "SMS HELP  SETUP  [rom.sms]",
            "Classic = raw SN76489  Modern = bandlimited PSG",
...
inline FieldAmmoEmu::SetupCtx ctx() noexcept {
    return {
        "AmmoSMS Setup", 14u, FieldAosAppIdentity::AppId::SmsSetup,
        active, page, sel, kPageCount, kPages,
        FieldAmmoSmsConfig::g,
        []() { FieldAmmoSmsConfig::load(); },
        []() { FieldAmmoSmsConfig::save(); },
        []() { FieldAmmoSmsConfig::setDefaults(); },
...
inline FieldAmmoSmsConfig::Options cfg;
inline std::uint8_t fb[FB_W * FB_H]{};
inline bool vgaMode13 = false;

using Audio = FieldAmmoEmu::AudioStream<15, active, audioLevel>;

inline void applyConfig(const FieldAmmoSmsConfig::Options& o) noexcept {
    cfg = o;
    AmmoSms::applyAudioConfig(chip, o.audioStyle, Audio::RATE);
}

inline bool loadRom(const char* path) {
    std::vector<std::uint8_t> data;
    if (!FieldAmmoFat::readFile(path, data) || data.size() < 256) return false;
    if (!AmmoSms::loadRom(chip, data.data(), data.size())) return false;
    applyConfig(cfg);
    romPath = path;
    vgaMode13 = false;
    return true;
...
inline bool findAnyRom(std::string& out) {
    if (FieldAmmoEmu::findRomInVfsDir("C:\\SMS", ".sms", out)) return true;
    return FieldAmmoEmu::findRomInVfsDir("C:\\SMS", ".bin", out);
}

inline void renderFrame(std::uint8_t* guestRam) noexcept {
    if (!guestRam) return;
    AmmoSms::renderFrame(chip, fb, FB_W, FB_H);
    if (!vgaMode13) {
...
inline void pushFrameAudio() noexcept {
    ...
    float block[kN]{};
    const int n = AmmoSms::renderAudioFrame(chip, block, kN);
...
inline void beginChrome(std::uint8_t* ram) noexcept {
    FieldWinApp::beginEmu(ram, " AmmoSMS ",
...
inline bool handleMenu(int action, std::uint8_t* ram) noexcept {
    ...
    case FieldWinFrame::A_FILE_OPEN:
        FieldAmmoEmu::openRomDialog(FieldAmmoEmu::Kind::Sms, cfg.lastRom.c_str());
        FieldEmuFileDialog::paint(ram);
        paused = true;
        return true;
    case FieldWinFrame::A_OPT_SETTINGS:
        FieldAmmoSmsSetup::open(true);
        optionsOpen = true;
        paused = true;
...
inline bool pumpOverlays(std::uint8_t* ram, std::uint16_t key, bool keyDown) noexcept {
    ...
    if (optionsOpen && FieldAmmoSmsSetup::active) {
        FieldAmmoSmsSetup::pump(ram, key, keyDown);
        if (!FieldAmmoSmsSetup::active) {
            optionsOpen = false;
            paused = false;
            cfg = FieldAmmoSmsConfig::g;
            applyConfig(cfg);
