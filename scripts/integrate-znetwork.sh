#!/usr/bin/env bash
# Integrate SG/ZNetwork (canonical outside lab) into NewLatest — pointers, build, lab posture.
set -euo pipefail
...
[[ -d "$CANON" ]] || {
  echo "ERROR: SG/ZNetwork missing at ${CANON}" >&2
  exit 1
}

link_if_missing "${CANON}" "${ROOT}/ZNetwork"
...
if [[ ! -x "${ZNETWORK_ROOT}/build/znetwork" ]]; then
...
# Best-of stack posture: relayer + smart_inside, never harm host OS, no sudo by default.
