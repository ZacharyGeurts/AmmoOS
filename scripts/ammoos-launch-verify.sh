...
if "$PY" "${ROOT}/lib/queen-ammoos-sovereignty.py" json 2>/dev/null | grep -q '"internet_pipe_percent": 100'; then
...
if "$PY" "${ROOT}/lib/field-local-dns-connect.py" json >/dev/null 2>&1; then
