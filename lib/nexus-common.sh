...
nexus_init_runtime_paths() {
  ...
  local use_local=0
  if nexus_is_dev_install; then use_local=1; fi
  if [[ "${NEXUS_FIELD_STANDALONE:-}" == "1" ]]; then use_local=1; fi
  ...
  if [[ "$(id -u)" -ne 0 ]] && [[ ! -w /var/log ]] 2>/dev/null; then use_local=1; fi
...
nexus_load_config() {
  if [[ -f "$NEXUS_CONFIG" ]]; then source "$NEXUS_CONFIG"; fi
  if [[ -f "${NEXUS_STATE_DIR}/settings.override" ]]; then source "${NEXUS_STATE_DIR}/settings.override"; fi
}
