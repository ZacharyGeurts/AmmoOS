...
def organize_monitor_panels(panels: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    doctrine = _load(DOCTRINE, {})
    pri_map = doctrine.get("monitor_panel_priority") or {}
    rows = []
    for i, p in enumerate(panels):
        pid = str(p.get("id") or "")
        rows.append({**p, "priority": float(pri_map.get(pid, len(panels) - i))})
    out, meta = _sort_rows(rows, context="recombinatorics", algorithm="composite_bsp")
    return out, meta
...
def apply_to_desktop(
    *,
    ...
    """Apply iron-plate organize to AmmoOS desktop slices."""
    ...
    panels = list((monitor or {}).get("panels") or [])
    sorted_panels, panel_meta = organize_monitor_panels(panels)
    monitor_out = {**(monitor or {}), "panels": sorted_panels, "iron_plate_sorted": True}
    ...
    return {
        ...
        "monitor_dashboard": monitor_out,
        "tray_icons": tray_out or tray_icons,
        "sort_meta": {
            "start_menu": cat_meta,
            "monitor_panels": panel_meta,
...
def build_panel(*, write: bool = True) -> dict[str, Any]:
    ...
    monitor = desktop.get("monitor_dashboard") or {}
    ...
    doc = {
        ...
        "surfaces": {
            ...
            "monitor_dashboard": {
                "iron_plate_sorted": (organized.get("monitor_dashboard") or {}).get("iron_plate_sorted"),
                "panel_ids": [p.get("id") for p in (organized.get("monitor_dashboard") or {}).get("panels") or []],
            },
