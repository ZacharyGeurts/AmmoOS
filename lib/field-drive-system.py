...
def _host_state_dir() -> Path:
    """Canonical host state — never the redirected field-drive path during publish."""
...
def field_roots() -> list[Path]:
    ...
    for p in (
        ...
        _host_state_dir() / "field-storage",
    ):
        if _safe_is_dir(p) and p not in roots:
            roots.append(p)
    media = Path("/media")
...
def system_dir() -> Path:
    return nexus_field_base() / "system"


def state_dir() -> Path:
    return nexus_field_base() / "state"
...
def _ignore_copy(_dir: str, names: list[str]) -> set[str]:
    skip = {".git", "__pycache__", ".pyc", "node_modules", ".tmp"}
...
def publish_system(*, full: bool = False) -> dict[str, Any]:
    ...
    if host_state.is_dir():
        for pattern in STATE_GLOBS:
            for src in host_state.glob(pattern):
                if not src.is_file():
                    continue
                dst = st_dst / src.name
                shutil.copy2(src, dst)
...
def sync_state_only() -> dict[str, Any]:
    ...
    if host_state.is_dir():
        for pattern in STATE_GLOBS:
            for src in host_state.glob(pattern):
                if src.is_file():
                    shutil.copy2(src, st_dst / src.name)
                    state_files.append(src.name)
