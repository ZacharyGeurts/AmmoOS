...
# Outside lab canonical copy first — SG/ZNetwork (NewLatest/znetwork is pointer-only).
if [[ -z "${ZNETWORK_ROOT:-}" ]]; then
  if [[ -d "${SG_ROOT:-}/ZNetwork" ]]; then
    ZNETWORK_ROOT="${SG_ROOT}/ZNetwork"
  elif [[ -d "${NEXUS_INSTALL_ROOT}/znetwork" ]]; then
...
nexus_znetwork_install_autostart() {
  ...
  boot_script="${ZNETWORK_ROOT:-${sg_root}/ZNetwork}/scripts/znetwork-boot.sh"
  release_install="${ZNETWORK_ROOT:-${sg_root}/ZNetwork}/scripts/znetwork-release-install.sh"
...
Exec=env SG_ROOT=${sg_root} NEXUS_INSTALL_ROOT=${NEXUS_INSTALL_ROOT} NEXUS_STATE_DIR=${NEXUS_STATE_DIR} ZNETWORK_ROOT=${ZNETWORK_ROOT:-${sg_root}/ZNetwork} DISPLAY=:0 DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/$(id -u)/bus bash ${boot_script}
Hidden=false
NoDisplay=true
...
nexus_znetwork_review_gate_script() {
  local candidate
  for candidate in \
    "${SG_ROOT}/ZNetwork/scripts/znetwork-review-gate.sh" \
