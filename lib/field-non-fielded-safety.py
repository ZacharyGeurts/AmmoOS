...
def defield_audit(*, via_converter: bool = True) -> dict[str, Any]:
    ...
    if publish_requires_defield() and not underlay_committed() and (nested_on_mirror > 0 or drive_nested):
        ok = False
    ...
    return {
        ...
        "updated": _now(),
        "ok": ok,
        "defield_ok": wrdt_restorable == 0 and int(local.get("field_tail_hits") or 0) == 0 and not drive_nested,
...
def gate_publish(*, op: str = "publish") -> dict[str, Any]:
    ...
    if audit.get("defield_ok") and not audit.get("nested_nexus_field_on_drives"):
        return {"ok": True, **audit}
