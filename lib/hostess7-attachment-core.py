...
def _vision_text_from_path(path: str) -> tuple[str, dict[str, Any]]:
    ...
    if fp.is_file() and hasattr(ocr, "ocr_image_path"):
        row = ocr.ocr_image_path(fp, label=f"attachment_{fp.stem}")
...
def _vision_text_look() -> tuple[str, dict[str, Any]]:
    ocr = _ocr_core()
    if ocr and hasattr(ocr, "final_eye_look"):
        look = ocr.final_eye_look(prefer="auto")
