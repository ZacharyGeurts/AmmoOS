...
def _ocr_images() -> list[dict[str, Any]]:
    ...
    session_img = Path(
        "/home/default/.grok/sessions/%2Fhome%2Fdefault/019ef500-f791-78f0-b600-4e86109c60b8/"
        "assets/image-a1cba4fe-9b45-469f-96b4-8157a1c9aaa5.jpg"
    )
    if session_img.is_file():
        dest = OCR_IMAGE_DIR / "true-reality-model.jpg"
        try:
            if not dest.is_file() or dest.stat().st_mtime < session_img.stat().st_mtime:
                dest.write_bytes(session_img.read_bytes())
        except OSError:
            pass
