...
(function () {
  ...
  function isAmmoOSFrame(frame) {
    try {
      const href = frame?.contentWindow?.location?.href || frame?.src || "";
      const u = new URL(href, location.origin);
      const loopback = u.hostname === "127.0.0.1" || u.hostname === "localhost";
      if (loopback && (u.pathname === "/field" || u.pathname.endsWith("/field"))) return true;
      const doc = frame?.contentDocument;
      if (doc?.documentElement?.dataset?.ammoosDesktop === "1") return true;
      if (doc?.body?.dataset?.ammoosDesktop === "1") return true;
      if (doc?.getElementById("hd-desktop")) return true;
    } catch (_) {}
    return false;
  }

  function injectAgent(frame) {
    if (isAmmoOSFrame(frame)) return false;
