...
No external Firefox/Chrome UI — operators see **Queen Browser** only. AmmoOS panel loads **inside** the RTX engine (`FieldWebPanel` + `QueenBoot.comp` compshader boot). Threat panel API auto-starts on `:9477` if needed.

Binary names: `queen-browser`, `field-queen`. Legacy engine binaries may still be named `fieldfox` on disk.
