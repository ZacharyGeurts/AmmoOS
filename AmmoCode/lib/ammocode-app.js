...
(function (global) {
  ...
  function isPagesStatic() {
    const cfg = global.AmmoCodeG16Config || global.AmmoCodeG16?.cfg?.() || {};
    return !!(
      cfg.pagesStatic ||
      document.documentElement.getAttribute("data-ammocode-pages") ||
      /github\.io$/i.test(String(location.hostname || ""))
    );
  }
