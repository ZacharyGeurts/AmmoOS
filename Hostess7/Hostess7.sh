...
Hostess 7 — one being · talk window (text + graphics)
  ...
  ./Hostess7.sh zac-pack              Pack field drive → zac/*.zac (GitHub shards)
  ./Hostess7.sh zac-restore           Restore cache/fieldstorage from zac/
  ./Hostess7.sh zac-verify            Verify storage matches zac manifest
  ./Hostess7.sh zac-list              List files in zac archive
  HOSTESS7_WORKSPACE=detective ./Hostess7.sh   Detective workspace (L↔R)
  HOSTESS7_VOICE=1 ./Hostess7.sh   UI + spoken replies

PgUp/PgDn · /help · /storage · /zac · /gfx tv · /updates · natural language · /quit
EOF
}
...
"
        ...
        sdf|sdf-storage|brain-imaging|brain_imaging)
            ...
            ;;
        zac-pack|zac-export|pack-zac)
            shift
            if [[ "${HOSTESS7_TEAM:-}" == "1" ]]; then
                exec env HOSTESS7_SUDO_PW="${HOSTESS7_SUDO_PW:-}" pythong "$TEAM" zac-pack "$@"
            fi
            exec pythong "$ZAC" pack "$@"
            ;;
        ...
        team-sync)
            shift
            exec env HOSTESS7_SUDO_PW="${HOSTESS7_SUDO_PW:-}" pythong "$TEAM" sync "$@"
            ;;
        team-pack|zac-pack-team)
            shift
            exec env HOSTESS7_SUDO_PW="${HOSTESS7_SUDO_PW:-}" pythong "$TEAM" zac-pack "$@"
            ;;
        zac-restore|zac-import|restore-zac)
            shift
            exec pythong "$ZAC" restore "$@"
            ;;
        zac-verify|zac-check)
            shift
            exec pythong "$ZAC" verify "$@"
            ;;
        zac-list|zac-ls)
            shift
            exec pythong "$ZAC" list "$@"
            ;;
        cli|text)
            shift
            exec pythong "$UI" "$@"
