...
SECURITY_DOMAINS: tuple[dict[str, str | tuple[str, ...]], ...] = (
    ...
    },
    {
        ...
        "body": (
            "Install once: sudo ./stealth_install.sh → nexus-genius.service. "
            "Modules: network-lockdown (no Samba/Avahi sharing), packet-oracle, threat-panel HTTPS :9477, "
            "firewall-sentinel (nftables), seal-vault (immutable backup), tamper-guard (auto-restore), "
            "Hostess7 bridge (truth corroboration). CLI: sg nexus -c 'nexus status|verify|panel'. "
            "Hostess7 commands: ./Hostess7.sh nexus status|verify|update|panel|corroborate."
        ),
    },
    {
    ...
    },
    {
        ...
        "body": (
            ...
            "Full brain: Codespaces or local with zac-restore. Internet gate: HOSTESS7_INTERNET=1 with truth filter. "
