...
PHASE 2 — EXECUTE (HOSTESS7_EXEC=1)
  ./Hostess7.sh self-update apply    run: reach scan → QA → zac-pack → git pull
  ./Hostess7.sh exec "git status"    one allowlisted OS command
  Talk: /self-update apply · /exec git status

PHASE 3 — SHIP INFINITE BRAIN (GitHub)
  ./Hostess7.sh zac-pack             pack cache/fieldstorage → zac/*.zac
  ./Hostess7.sh zac-verify           checksum verify
  Commit zac/ shards — restore on clone with zac-restore
...
LOOP: updates → plan → apply → zac-pack → verify QA → explore → teach flow → repeat.
...
EXPLORATION_QUEUE = (
    ...
    {"query": "Pack the field drive for GitHub — what would zac-pack produce?"},
...
def write_brief() -> Path:
    ...
    with THOUGHTS.open("a", encoding="utf-8") as f:
        f.write(json.dumps({
            ...
            "text": "Self-update brief installed. Loop: updates → plan → apply → zac-pack → QA → explore.",
        }) + "\n")
        f.write(json.dumps({
            "ts": _ts(),
            "kind": "arc",
    ...
    with DIRECTIVES.open("a", encoding="utf-8") as f:
        f.write(json.dumps({
            "ts": _ts(),
            "lane": "hostess",
            "task": "Begin self-update loop: ./Hostess7.sh self-update plan then apply. Then zac-pack and explore memes/reach.",
            "priority": "P1",
        }) + "\n")
