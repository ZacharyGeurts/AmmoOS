#!/usr/bin/env pythong
"""TEAM field drive — fast NVMe ops for Hostess7 (zac-pack, library, bulk).
...
def pull_zac_from_team() -> Path:
    """Copy zac shards from TEAM Hostess7 back to local repo for git push."""
    src = team_hostess7() / "zac"
    dest = ROOT / "zac"
    if not src.is_dir():
        raise FileNotFoundError(f"no zac on team drive: {src}")
    dest.mkdir(parents=True, exist_ok=True)
    _rsync(src, dest, excludes=[])
    return dest
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_p.add_argument("command", help="Shell command e.g. ./Hostess7.sh zac-pack --jobs 8")

    sub.add_parser("pull-zac", help="Pull zac/ from TEAM to local repo")
    pack_p = sub.add_parser("zac-pack", help="zac-pack on TEAM then pull shards")
    pack_p.add_argument("--jobs", type=int, default=8)
    ...
    if args.cmd == "pull-zac":
        pull_zac_from_team()
        print(f"OK pull-zac → {ROOT / 'zac'}")
        return 0

    if args.cmd == "zac-pack":
        os.environ.setdefault("HOSTESS7_SUDO_PW", os.environ.get("HOSTESS7_SUDO_PW", ""))
        sync_to_team()
        force = " --force" if args.force else ""
        rep = run_on_team(f"./Hostess7.sh zac-pack --jobs {args.jobs}{force}")
        print(rep["stdout"])
        if not rep["ok"]:
            print(rep["stderr"], file=sys.stderr)
            return 1
        pull_zac_from_team()
        print("OK team zac-pack + pull-zac")
        return 0
