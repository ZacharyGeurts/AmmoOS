...
FLOW_LAYERS: tuple[dict[str, object], ...] = (
    ...
    },
    {
        ...
        "body": (
            ...
            "ZAC7 packs the whole drive for GitHub (`zac-pack` / `zac-restore`)."
        ),
        "commands": (
            ...
            "./Hostess7.sh zac-pack",
        ),
        "docs": ("README.md", "scripts/field_zac.py"),
    },
    {
    ...
    },
    {
        ...
        "tags": ("self-update", "code", "edit", "scripts", "qa", "zac", "git", "exec"),
        "body": (
            ...
            "`./Hostess7.sh self-update apply` runs reach scan → QA scripts → zac-pack → git pull. "
    ...
    },
    {
        ...
        "body": (
            ...
            "Full brain restore from shards: `./Hostess7.sh zac-restore`. Verify: "
            "`pythong scripts/qa_hostess_turing_test.py` and corpus QA tests."
        ),
        "commands": (
            "./Hostess7.sh off",
            "./Hostess7.sh on",
            "./Hostess7.sh zac-restore",
...
def synthesize_flow_paragraphs(query: str) -> list[str]:
    ...
    if any(k in q_low for k in ("self-update", "restart", "edit", "code", "apply")):
        paras.append(
            "Self-update loop: updates → plan → apply (HOSTESS7_EXEC=1) → zac-pack → QA → "
