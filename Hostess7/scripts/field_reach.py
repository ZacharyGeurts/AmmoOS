...
SCAN_SUBDIRS = (
    ...
    "build", "zac", "cache/fieldstorage/brain",
)
...
def self_update_steps(*, apply: bool = False) -> list[dict[str, Any]]:
    """Truth-ordered self-update pipeline — scan → QA → zac → git."""
    h = hostess7_root()
    ...
    qa_scripts = [
        ...
        h / "scripts" / "qa_field_zac_test.py",
    ...
    if (h / "scripts" / "field_zac.py").is_file():
        steps.append({
            "id": "zac-pack",
            "action": "pack field drive for GitHub",
    ...
    if apply:
        ...
        for step in steps:
            ...
            if step.get("id") == "zac-pack" and agents_script.is_file():
                try:
                    off = subprocess.run(
                    ...
                    results.append({
                        "step": "agents-off",
                        "ok": True,
                        "note": "stopped agent daemon before zac-pack",
                    })
                except OSError as exc:
            ...
            if step.get("id") == "zac-pack" and agents_were_on and agents_script.is_file():
                try:
                    on = subprocess.run(
                    ...
                    results.append({
                        "step": "agents-on",
                        "ok": on.returncode == 0,
                        "note": "restarted agent daemon after zac-pack",
                    })
                except OSError as exc:
