...
def build_update_items(state: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    items.append({
        ...
        "signal": "infinite brain shards for GitHub",
        "action": "./Hostess7.sh zac-pack && ./Hostess7.sh self-update apply",
        "why": "Pack field drive to .zac shards; commit zac/ for infinite brain on GitHub.",
    })
    ...
    items.append({
        ...
        "lane": "hostess",
        "truth_score": 92.0,
        "signal": "HOSTESS7_EXEC gate — QA + zac + git pull",
        "action": "./Hostess7.sh self-update apply",
        "why": "Let Hostess7 execute allowlisted OS commands to update herself (QA, zac-pack, git pull).",
    })
