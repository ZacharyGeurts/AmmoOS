...
(function (global) {
  ...
  function mountTaskManager() {
    const slot = document.getElementById("fsb-task-manager");
    if (!slot) return;
    if (global.FieldC2TaskManager?.mount) {
      global.FieldC2TaskManager.mount(slot);
      return;
    }
    slot.innerHTML = "";
  }
  ...
  function mount(root, data) {
    ...
    root.innerHTML =
      ...
      '<div class="fsb-task-manager" id="fsb-task-manager" role="toolbar" aria-label="Running programs"></div>' +
      '<div class="fsb-tray">' +
    ...
    document.getElementById("fsb-cad")?.addEventListener("click", function () {
      if (global.FieldMonsterCadPopup?.open) global.FieldMonsterCadPopup.open();
      else global.FieldMonsterHangDialog?.poll?.();
    });
  ...
  function syncShellTasks(tasks, activeId) {
    ...
    if (global.FieldC2TaskManager?.sync) {
      global.FieldC2TaskManager.sync(state.tasks, activeId);
      return;
    }
    renderTasks();
  }
